...
# an item does not have enough people rate it
# instead of filling the ratings with 0
# we can find its K nearest neighbors from its features
# get the the ratings from avering the people in its KNN
# then use svd on the new rating matrix,
# we can predict the ratings from each people to each movie
...
class CFmodel():
	...
	# can be improved using mask, SGD
	...
	def train(self, ratingsMat, itemFeatureTable):
		...
		self.knnModel.fit(itemFeatureTable) # only use similar items to fit my rating
		...
		for case in rareCases:
			if case+1 in itemFeatureTable.index:
				...
				target_ratings = ratingsMat[:,neighborPos].sum(axis=1).astype(float)/(target_count+1e-6)
				#nonzero mean
